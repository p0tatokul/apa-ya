...
function iHeartYou(){
	...
	if(window.location.hash){
		...
		vtext.textContent = "hehhehe i love u more";
	}
...
function linkChangey(){
	if(yourMessage.value==""){
		yourLink.value = "http://p0tato.rf.gd/door/";
	}else{
		yourLink.value = "http://p0tato.rf.gd/door/#"+encodeURIComponent(encryptString(yourMessage.value));
	}
};
...
function socialShare(event,type){
	...
	var title = "apa ya";
	var url = "https://p0tato.rf.gd/door/share.php?type="+type+"&link="+encodeURIComponent(link);
	var width = 640;
	var height = 480;
	...
	switch(type){
		case "facebook":
			...
			url += "&t="+encodeURIComponent("A lovely message for all my dear friends. This minigame only takes a minute to play, check it out! apa ya --");
			width = 626;
			height = 436;
			break;
		case "twitter":
		...
		case "tumblr":
			...
			url += "&name="+encodeURIComponent("apa ya");
			url += "&description="+encodeURIComponent("A lovely message for all my dear followers, in this 1-min minigame.");
			width = 446;
			height = 430;
			break;
		case "reddit":
			window.open('http://www.reddit.com/submit?v=5&amp;noui&amp;jump=close&amp;url='+encodeURIComponent(link)+'&amp;title='+encodeURIComponent("apa ya: a one-minute minigame"), "reddit",'toolbar=no,width=700,height=550');
			return false;
			break;
		case "stumbleupon":
